...
public class TypeParameterResolver {
  ...
  /**
   * @param typeVar 需要解析的Type类型
   * @param srcType 要解析的Type类型的起始Type类型
   * @param declaringClass 字义TypeVar的Class类型
   * @param clazz srqType的Class类型
   * @param superclass 父类的Type类型
   * @return
   */
  ...
  private static ParameterizedType translateParentTypeVars(ParameterizedType srcType, Class<?> srcClass,
    ...
    // 继承的父类型中定义的泛型参数（不是原始类中的泛型变量）
    Type[] parentTypeArgs = parentType.getActualTypeArguments();
    // 起始的Type类型中定义的泛型参数
    Type[] srcTypeArgs = srcType.getActualTypeArguments();
    // 起始的Type类型中的泛型变量（从class中获取的，是Class类中定义的泛型变量）
    TypeVariable<?>[] srcTypeVars = srcClass.getTypeParameters();
    Type[] newParentArgs = new Type[parentTypeArgs.length];
    boolean noChange = true;
    for (int i = 0; i < parentTypeArgs.length; i++) {
      // 如果父类型参数是TypeVariable，那么需要找到对应的TypeVariable在srcType中的索引
